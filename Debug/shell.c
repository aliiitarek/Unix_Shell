...
char* getEnvVar(char* envName){
	...
	if(envName[0]=='$'){
		...
		int idx = 0;
		for(int i=1;envName[i]!='\0';i++) envTemp[idx++] = envName[i];
...
char** parser(char* input, char split) {
	...
	while (ip < size_ip) {
		while (ip < size_ip && input[ip] != split) {
...
//				printf("%s %s\n",envVar,enviromentTry); // checker
	...
	parsed[arg] = NULL;
	if(split==' '){
		for(int i=0;parsed[i]!=NULL;i++){ /*replacing enviroment variables with their values*/
			if(parsed[i][0]=='$') {
				char* envName;
				envName = (char*) malloc(max_size_arg*(sizeof(char)));
				int envName_idx = 0;
				for(int j=1;parsed[i][j]!='\0';j++) envName[envName_idx++] = parsed[i][j];
	...
	while(1){
		...
		if(areEqual(parsed[0],"cd")){
			...
			if(argSize==2 && parsed[1][0]=='~'){ /* set ~ to /home */
				...
				strcpy(tempo,"/home");
				int tempo_idx = 5;
				for(int i=1;parsed[1][i]!='\0';i++) tempo[tempo_idx++] = parsed[1][i];
